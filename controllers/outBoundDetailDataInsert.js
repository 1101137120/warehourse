...
exports.postOutBoundDetails = function(req, res,next) {
	...
	var detailSQL = "INSERT INTO outbound_detail (product_id,amount,state,isDelete,outboundId,updateTime,remarks) VALUES ";
	var detailDataSQL = "";
	for(var i=0;i<detailData.length;i++)
	{
		...
		detailDataSQL  = detailDataSQL + "(" +detailData[i].productID + "," +detailData[i].amount + "," +detailData[i].state + "," +"0" + ",'" +detailData[i].outboundId + "','" +detailData[i].updateTime + "',"+detailData[i].remarks+")";
	}
	detailSQL = detailSQL +detailDataSQL+";";
		...
		outBoundDetailInstance.query(detailSQL,function(err, rows, fields) {
			...
			else
			{
				console.log("ININININssd");
								...
								apiOutput.message = "detail Insert";
								apiOutput.response = rows;			
								res.json(apiOutput);	*/
			var sql = "INSERT INTO outboundorder (id,date,time,workerId,state,isDelete,updateTime) VALUES ("+id+",'"+date+"','"+time+"','"+workerId+"',"+state+",0,'"+updateTime+"');";
			console.log("ININININ" + sql);
			outBoundOrderInstance.query(sql,function(err, Orderrows, fields) {
				console.log("ININININ" + Orderrows);
			if(err) 
			{
				console.log(JSON.stringify(err));
				customErr.status = 503;
