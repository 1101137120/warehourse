...
exports.postInBoundDetails = function(req, res,next) {
	...
	{
		var sql = "Update inboundorder SET workerId = '"+workerId+"' WHERE id = '"+id+"';";
		inBoundOrderInstance.query(sql,function(err, rows, fields) {
			...
			var sql = "Update inbound_detail SET isDelete = 1,updateTime = '"+updateTime+"' WHERE inboundId = "+id+";";
		...
		inBoundDetailInstance.query(sql,function(err, rows, fields) {
			...
			{
				var detailSQL = "INSERT INTO inbound_detail (product_id,amount,state,isDelete,inboundId,updateTime) VALUES ";
				var detailDataSQL = "";
				for(var i=0;i<detailData.length;i++)
				{
					if(detailDataSQL!="")
						...
						detailDataSQL  = detailDataSQL + "(" +detailData[i].productID + "," +detailData[i].amount + "," +detailData[i].state + "," +"0" + ",'" +detailData[i].inboundId + "','" +detailData[i].updateTime + "')";
				}
					detailSQL = detailSQL +detailDataSQL+";";
					inBoundDetailInstance.query(detailSQL,function(err, Orderrows, fields) {
