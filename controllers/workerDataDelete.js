...
exports.postWorkers = function(req, res,next) {
	...
	var peopleID = JSON.parse(req.body.peopleID);
	if(!peopleID)
	{
	...
	{
		if(!sqlwhere)
			sqlwhere = "id = "+peopleID[i];
		else
			sqlwhere = sqlwhere + " or id = "+peopleID[i];
	}
	...
	else
	{
		...
		var sql = "UPDATE worker SET isDelete = 1  WHERE "+sqlwhere+" ;";
		workerInstance.query(sql,function(err, rows, fields) {
			...
			else
			{
					...
					if(rows.length!=0){
								...
								apiOutput.message = "people Delete";
							...
							apiOutput.message = "people not Delete";
							apiOutput.response = rows;			
							res.json(apiOutput);	
