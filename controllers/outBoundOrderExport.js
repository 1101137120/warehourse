...
exports.postOutBoundOrders = function(req, res,next) {
	...
	{
				var sql = "select outboundorder.id,CONCAT(outboundorder.date, ' ', outboundorder.time) as datetime,name as workerName,remarks as orderRemarks,state as orderState from outboundorder,worker where outboundorder.id IN("+sqlwhere+") and outboundorder.workerId = worker.id";
