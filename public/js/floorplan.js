...
var tag_name="未命名";
	...
	socket.on("channel1", function (readerObj) {
		...
		if(!$("#tabletagdelay1h").children('td').val()){
		$("#trnull1h").remove();
			$("#tabletagdelay1h").append("<tr id='trnull1h'>"+
						"<td data-field='tag_uid'>目前無人員資料</td>"+
					"</tr>");	
		}
			
		if(!$("#tabletagdelay3h").children('td').val()){
		$("#trnull3h").remove();
			$("#tabletagdelay3h").append("<tr id='trnull3h'>"+
						"<td data-field='tag_uid'>目前無人員資料</td>"+
					"</tr>");
		}
		
		if(!$("#tabletagdelay3d").children('td').val()){
		$("#trnull3d").remove();
			$("#tabletagdelay3d").append("<tr id='trnull3d'>"+
						"<td data-field='tag_uid'>目前無人員資料</td>"+
					"</tr>");
		}		
		...
		$("#latesetTime").text("最後更新時間"+formatDate);
